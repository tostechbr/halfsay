...
@main @MainActor struct Halfsaid {
    static let usage = """
    usage: halfsaid [--text "sentence"] [--dry-run] [--locale en-US] [--wpm 160]
    ...
    static func main() async {
        ...
        let session = Session(client: JevClient(apiKey: key), apps: apps, executor: Executor(apps: apps, dryRun: options.dryRun))
        if let text = options.text {
            ...
            await listen(session, options: options, hints: apps.keys.sorted())
        }
    }

    static func listen(_ session: Session, options: Options, hints: [String]) async {
