...
The most useful report is a sentence partway got wrong, with its trace:

1. Run it with `--log`: `open build/partway.app --args --log`, or `swift run partway --log`.
2. Say the sentence, then open the newest file in `~/Library/Logs/partway/`.
3. Open an issue with the lines for that sentence. The log holds everything the mic heard, so cut anything private first.

## Changing the code

- `make test` has to pass, and new logic comes with a test in `Tests/`: the decision logic in `PartwayCore` runs offline.
...
Much of partway was written with [Claude Code](https://claude.com/claude-code); those commits say so.
